...
def generate_password():                                                            # declaration of a function to generate a random password in python
    try:                                                                            # read user inputs, code will only execute if both length and repeat are filled
...
# Define a string containign letters, symbols and numbers
...
title_label.pack()
# Read length
length_label = Label(password_gen, text="Enter a length of password: ")
length_label.place(x=20, y=30)
length_entry = Entry(password_gen, width=3)
length_entry.place(x=190, y=30)
# Read repitiion
repeat_label = Label(password_gen, text="Repitition? 1: no repetition, 2: otherwise: ")
