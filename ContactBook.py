...
root = Tk()                                     # intializing root window for contact book 
root.geometry('700x550')                        # sets dimension of the window
root.config(bg = '#d3f3f5')                     
root.title('PYthonGeeks Contact Book')
root.resizable(0,0)
contactlist = [                                 # creates 'database' of contacts in form of a list
...
""" Step 4 - Function to get a select value"""
def Selected():                                 # Function used to select values
    print("Hello", len(select.curselection()))  # curselection is part of tkinter. It is used to display selected items
    if len(select.curselection())==0:           # counts length of data. If length is zero, error message is shown.
...
def AddContact():                               # Function used to create new contacts
    if Name.get() !="" and Number.get() !="":   # returns value of name and number w/ key
        ...
        messagebox.showerror("Error", "Please fill the infomation")
...
""" Step 6 - Function to delete and view contact"""
def Delete_Entry():
...
""" Step 8 - Define buttons labels and entry widget"""
